...
/**
 * For backwards compatibility, define an alias to the old name of the
 * class. TODO(mesch): Follow up with external clients to make them
 * change their names too.
 * @type Function
 */
var JsExprContext = JsEvalContext;
...
function jsEvalToFunction(expr) {
  if (!JsEvalContext.evalToFunctionCache_[expr]) {
    try {
      ...
      // cf. <http://mesch.nyc/xbrowser/test-function-versus-eval>.
...
function jsEvalToValues(expr) {
  // TODO(mesch,schuck): It is insufficient to split the values by
  // simply finding semi-colons, as the semi-colon may be part of a
  // string constant or escaped. This needs to be fixed. NOTE(mesch):
  // For setting style on the node, however, compare the NOTE(mesch)
  // below.
  var ret = [];
