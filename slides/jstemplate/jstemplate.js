 ...
 * IE6. TODO(mesch): So far, these are only used here, but we could
 * use them thoughout the code and thus move them to constants.js.
 */
var CHAR_asterisk = '*';
var CHAR_dollar = '$';
var CHAR_period = '.';
 ...
 * NOTE(appleton): This is necessary to process deep templates in
 * Safari which has a relatively shallow maximum recursion depth of
 * 100. See <http://mesch.nyc/xbrowser/test-js-recursion-depth.html>.
 * @class
 * @constructor
 */
...
JstProcessor.prototype.run_ = function(f) {
  ...
  /**
   * The index into the queue for each depth.
   * NOTE(appleton): I experimented with maintaining the queues in
   * reverse order,\ (popping off of the end) but the repeated calls to
   * .pop() consumed 90% of this function's execution time.
   * @type Array.<number>
   */
 ...
 *
 * NOTE(appleton): This method takes ownership of the given array!
 *
...
JstProcessor.prototype.jstProcessInner_ = function(context, template) {
  ...
  // into jstemplates.
  //
  // NOTE(mesch): Evaluation order is sometimes significant, e.g.  in
  // addressbook_jstemplate_html.tpl, the expression evaluated in
  // jseval relies on the values set in jsvalues, so it needs to be
  // evaluated *after* jsvalues. TODO(mesch): This is quite arbitrary,
  ...
  if (content) {
    ...
    // NOTE(appleton): Newly generated children should be ignored, so we
    // explicitly store the children to be processed.
...
JstProcessor.prototype.jstSelect_ = function(context, template, select) {
  ...
  // NOTE(mesch): value instanceof Array is occasionally false for
  ...
  if (multiple) {
    if (multipleEmpty) {
      ...
      // replicating the last template instance. NOTE(mesch): If the
      ...
      // NOTE(stse): The following comment is now obsoleted unless we
      // change the return type of the function 'domGetAttribute' in
      // dom.js: For completeness, in case any browser returns undefined
      // and not null for getAttribute of nonexistent attributes, we also
      // test for === undefined.
...
JstProcessor.prototype.jstValues_ = function(context, template, values) {
  for (var i = 0, I = jsLength(values); i < I; i += 2) {
    ...
    if (label.charAt(0) == CHAR_dollar) {
      ...
      // using jsvalues. NOTE(mesch): Setting the style attribute as
      // attribute is currently difficult because of the
      // TODO(mesch,schuck) in jstGetAttributeValues(), but would
      // probably not be good style anyway.
